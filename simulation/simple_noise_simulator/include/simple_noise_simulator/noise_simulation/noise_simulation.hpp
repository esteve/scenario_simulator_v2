...
#include <simple_noise_simulator/noise_simulation/detection_noise/detection_noise.hpp>
...
public:
    ...
    const simulation_api_schema::LidarConfiguration & configuration, rclcpp::Node & node) -> void
  {
    if (configuration.architecture_type() == "awf/universe") {
      lidar_noises_.push_back(std::make_unique<LidarSensor<sensor_msgs::msg::PointCloud2>>(
...
private:
  std::vector<std::unique_ptr<LidarSensorBase>> lidar_noises_;
  std::vector<std::unique_ptr<DetectionSensorBase>> detection_noises_;
};
}  // namespace simple_noise_simulator
