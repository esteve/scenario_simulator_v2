...
#include <memory>
#include <simple_noise_simulator/noise_simulation/noise_simulation.hpp>
#include <string>
#include <vector>
  ...
  const std::vector<traffic_simulator_msgs::EntityStatus> & status)
{
  ...
  for (auto & noise : lidar_noises_) {
    noise->update(current_time, status, current_ros_time);
    const auto objects = noise->getDetectedObjects();
  ...
  for (auto & noise : detection_noises_) {
    noise->update(current_time, status, current_ros_time, lidar_detected_objects);
  }
}
}  // namespace simple_noise_simulator
